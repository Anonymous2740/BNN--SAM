...
class FPN_BiReal_Attention(nn.Module):
    ...
    def __init__(self,
                 ...
                 conv_cfg=dict(type='BinarizeConv2d'),
                 norm_cfg=None,
                 act_cfg=None,
                 upsample_cfg=dict(mode='nearest')):
        super(FPN_BiReal_Attention, self).__init__()
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            self.encoder_att_block_extra = nn.ModuleList([nn.ModuleList([self.conv_layer([out_channels,out_channels])])])
            for j in range(2):
                if j < 1:
                    self.encoder_att_block_extra.append(nn.ModuleList([self.conv_layer([out_channels,out_channels])]))
    ...
    def forward(self, inputs):
        ...
        outs_attention_cls = [
            self.encoder_att_block_extra[0][0]((self.fpn_convs[i](laterals[i])) * self.encoder_att_extra[0][0](laterals[i])) for i in range(used_backbone_levels)
        ]
        outs_attention_loc = [
            self.encoder_att_block_extra[1][0]((self.fpn_convs[i](laterals[i])) * self.encoder_att_extra[1][0](laterals[i])) for i in range(used_backbone_levels) 
        ]

        # return outs_attention_cls, outs_attention_loc, outs
        ...
        if self.num_outs > len(outs):
            ...
            if not self.add_extra_convs:
                for i in range(self.num_outs - used_backbone_levels):
                    ...
                    outs_attention_cls.append(F.max_pool2d(outs_attention_cls[-1], 1, stride=2))
                    outs_attention_loc.append(F.max_pool2d(outs_attention_loc[-1], 1, stride=2))
