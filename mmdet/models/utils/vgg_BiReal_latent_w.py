...
from .irnet.binaryfunction_latent_w import BinarizeConv2d,BiLineMaxPool2d,BiLineConv2d
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        "x and real_x are the result of binary conv and conv"

        x, real_x = self.conv(x)
        x = self.bn(x)
        real_x = self.bn(real_x)

        if self.downsample is not None:
            residual_b = self.downsample(residual_b)
            residual_w = self.downsample(residual_w)
...
class VGG(nn.Module):
    ...
    def forward(self,x):
        ...
        outs = []
        real_outs = []
        x, real_x = self.vgg_layers(x)
        outs.append(x)
        real_outs.append(real_x)
        return outs, real_outs        
