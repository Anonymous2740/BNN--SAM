...
from .irnet.binaryfunction import BinarizeConv2dBop
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False,
        ...
        self.binconv = BinarizeConv2dBop(in_channels, out_channels,
...
class BasicNoDownsampleBlock(nn.Module):

    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, bias=False,
                 downsample=None):
        super(BasicNoDownsampleBlock, self).__init__()
        self.binconv = BinarizeConv2dBop(in_channels, out_channels,
...
def make_vgg_layer(cfg, i=3, bias=False):
    ...
    binconv6 = BasicNoDownsampleBlock(512, 1024, kernel_size=3, padding=6, dilation=6, bias=bias)
    ...
    layers += [pool5]
    layers += [binconv6, nn.BatchNorm2d(1024), nn.ReLU(inplace=True)]#conv6进行了二值化，但是用到的激活函数是Relu
