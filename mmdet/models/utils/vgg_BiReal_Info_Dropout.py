...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False,
        ...
        self.info_Dropout = Info_Dropout(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, if_pool=False)
    ...
    for j in range(len(cfg)):
        ...
        if v == 'M':#并不是把maxpool层去掉了，而是进行了替换，换成了Conv2d(stride=2)
            ...
            if in_channels == i:#第一层不进行二值化
                ...
                #new add
                info_Dropout = Info_Dropout(in_channels, v, kernel_size=3, stride=1, padding=1, if_pool=False)
                layers += [info_Dropout]
                # ##
            else:
...
class Info_Dropout(nn.Module):
    ...
    def forward(self, x_old, x):
        ...
        with torch.no_grad():
            ...
            probdbg = (self.all_one_conv_radius_wise(distance) / self.patch_sampling_num) ** (1 / self.temperature)
            prob.to('cpu')
            if self.if_pool:
            ...
            random_choice = self.random_sample(prob, sampling_num=int(self.drop_rate * h * w))
        ...
        return (x * random_mask.view(x.shape)).cuda()
        
    def random_sample(self, prob, sampling_num):
        
        batch_size, channels, h, w = prob.shape
        return torch.multinomial((prob.view(batch_size * channels, -1) + 1e-8), sampling_num if sampling_num>0 else 1, replacement=True)   