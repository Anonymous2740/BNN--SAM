...
# Repvgg论文:https://arxiv.org/pdf/2101.03697.pdf
# Repvgg代码: https://github.com/DingXiaoH/RepVGG
# 对应的权重文件在Repvgg代码中
...
def create_RepVGG_A0_S(deploy=False):
    return RepVGG(num_blocks=[2, 4, 8, 1], num_classes=1000,
                  width_multiplier=[0.5, 0.5, 0.5, 0.5], override_groups_map=None, deploy=deploy)
...
'RepVGG-A0-S': create_RepVGG_A0_S,
...
class Repvgg_X(nn.Module):
    def __init__(self,
                 repvgg_name = 'RepVGG-A0',
                 pretrained=None,
                 init_cfg=None,
                 deploy=False):
        super(Repvgg_X, self).__init__()
        repvgg_build_func = get_RepVGG_func_by_name(repvgg_name)
...
if __name__ == "__main__":
    ...
    model = Repvgg_X(repvgg_name = 'RepVGG-A0-S', deploy=True)
    out=model(x)
    print(model)
